...
// ✅ FIXED: moved down so it never overlaps the stats text
function uiShareRect_GameOver() {
  const w = 170, h = 50;
  return { x: (W - w) / 2, y: H * 0.74, w, h };
}
...
function draw() {
  ...
  if (!Game.running) {
    ...
    // ✅ FIXED spacing so Share button never overlaps any text
    ctx.font = "16px system-ui";
    ctx.fillText(`Mode: ${MODES[Game.mode].label}`, W / 2, H * 0.48);
    ctx.fillText(`Score: ${Game.score}`, W / 2, H * 0.53);
    ctx.fillText(`Best: ${Game.best}`, W / 2, H * 0.58);
    ctx.fillText(`Close Calls: ${Game.closeCalls}`, W / 2, H * 0.63);
    ctx.fillText(`Max Streak: ${Game.maxStreak}`, W / 2, H * 0.68);

    const sr = uiShareRect_GameOver();
    drawButton(sr, "Share Score");

    ctx.fillStyle = "rgba(255,255,255,0.85)";
    ctx.fillText("Tap anywhere else to restart", W / 2, H * 0.84);
    ctx.textAlign = "left";
  }

...
function setMode(modeKey) {
  ...
  Game.mode = modeKey in MODES ? modeKey : "easy";
  LANES = m.lanes;
  SPEED_INCREMENT = m.speedInc;

  // reset player safely inside lane range
...
// ✅ Difficulty buttons on start menu
function uiModeButtons() {
  const w = Math.min(260, W - 64);
  const h = 48;
  const gap = 12;
  const totalH = h * 3 + gap * 2;
  const top = H * 0.52 - totalH / 2;

  return {
    easy:   { x: (W - w) / 2, y: top + (h + gap) * 0, w, h },
    medium: { x: (W - w) / 2, y: top + (h + gap) * 1, w, h },
    hard:   { x: (W - w) / 2, y: top + (h + gap) * 2, w, h },
  };
}
...
document.addEventListener("keydown", (e) => {
  ...
  // ✅ Start screen: choose difficulty quickly
  if (Game.running && !Game.started) {
    if (e.key === "1") { setMode("easy"); startGameNow(); playClick(); return; }
    if (e.key === "2") { setMode("medium"); startGameNow(); playClick(); return; }
    if (e.key === "3") { setMode("hard"); startGameNow(); playClick(); return; }

    const k0 = e.key;
    if (!["Shift", "Control", "Alt", "Meta"].includes(k0)) {
      // default: start with current selected mode
      startGameNow();
      playClick();
      return;
    }
...
canvas.addEventListener("pointerdown", async (e) => {
  ...
  // ✅ Start menu: tap difficulty button
  if (Game.running && !Game.started) {
    const mb = uiModeButtons();
    if (inRect(x, y, mb.easy))   { setMode("easy");   startGameNow(); playClick(); return; }
    if (inRect(x, y, mb.medium)) { setMode("medium"); startGameNow(); playClick(); return; }
    if (inRect(x, y, mb.hard))   { setMode("hard");   startGameNow(); playClick(); return; }

    // Tap anywhere else: start with currently selected mode
    startGameNow();
    playClick();
    return;
  }
...
function draw() {
  ...
  ctx.fillText(`Mode: ${MODES[Game.mode].label}  •  Speed x${Game.speedMul.toFixed(2)}  (+${Math.round(SPEED_INCREMENT*100)}% / 10 pts)`, 16, 74);
  ...
  if (Game.running && !Game.started) {
    ...
    // Difficulty buttons
    const mb = uiModeButtons();
    drawButton(mb.easy,   "Easy",   "1 • 3 lanes • +10% / 10 pts");
    drawButton(mb.medium, "Medium", "2 • 4 lanes • +15% / 10 pts");
    drawButton(mb.hard,   "Hard",   "3 • 5 lanes • +30% / 10 pts");

    ctx.fillStyle = "rgba(255,255,255,0.85)";
    ctx.textAlign = "center";
    ctx.font = "14px system-ui";
    ctx.fillText("Tap a difficulty to start", W / 2, H * 0.78);
    ctx.fillText("Arrow keys / A-D move • Space cycles • P pause • R restart", W / 2, H * 0.82);
    ctx.textAlign = "left";
  }

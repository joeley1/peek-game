...
// ✅ NEW: streak expires after this many seconds without a new close call
...
function update(dt) {
  ...
  // ✅ NEW: streak timer countdown (even if no close call)
  ...
  for (const o of obstacles) {
    ...
    if (!o.closeEvaluated && o.y > Player.y + 30) {
      ...
      if (o.wasSameLaneEver && o.maxOverlap >= closeLow && o.maxOverlap < killOverlap) {
        ...
        // ✅ NEW: refresh streak timer on each close call
      ...
      // ❌ IMPORTANT: no instant streak reset here anymore (timer handles expiry)
    }
  }
...
function draw() {
  ...
  // HUD (Best removed from top-right)
  ctx.fillStyle = "#fff";
  ctx.textAlign = "left";
  ...
  // Game over overlay (still shows Best here)
  if (!Game.running) {
